# -*- coding: utf-8 -*-
#Hyperparameter optimization for models with rectangle topology
import os
import h5py
import optuna
...
# Data loading
# ----- ----- ----- ----- ----- -----
f = h5py.File("shared_drive/vdp_data_dt.hdf5", "r")
keys = list(f.keys())
print(keys)
...
def objective(trial):
    ...
    hidden_layers = trial.suggest_int( "hidden_layers", 0, 10)
    neurons_per_layer = trial.suggest_int( "neurons_per_layer", 5, 100)
    model   = MLPs.VariableBoxMLP(x_trn.shape[-1], y_trn.shape[-1], neurons_per_layer, hidden_layers)
...
study_name = "box_nn_final"  # Unique identifier of the study.
...
fig = optuna.visualization.plot_contour(study, params=["hidden_layers", "neurons_per_layer"])
fig.write_html(str(study_name)+"_cont1.html", include_plotlyjs="cdn")
'''fig = optuna.visualization.plot_contour(study, params=["lr", "neurons_per_layer"])
fig.write_html(str(study_name)+"_cont2.html", include_plotlyjs="cdn")
fig = optuna.visualization.plot_contour(study, params=["lr", "hidden_layers"])
fig.write_html(str(study_name)+"_cont3.html", include_plotlyjs="cdn")'''
